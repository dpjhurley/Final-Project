...
export default class MainDisplay extends Component {
    ...
    handleBrandCheck = e => {
        if (this.state.filterByBrand.includes(e.target.value)) {
            this.setState({
                filterByBrand: this.state.filterByBrand.filter(
    ...
    handleCategoryCheck = e => {
        if (this.state.filterByCategory.includes(e.target.value)) {
            this.setState({
                filterByCategory: this.state.filterByCategory.filter(
                    category => category !== e.target.value
                )
            });
        } else {
            this.setState({
                filterByCategory: this.state.filterByCategory.concat(
                    e.target.value
                )
            });
        }
    ...
    handleColorCheck = e => {
        if (this.state.filterByColor.includes(e.target.value)) {
            this.setState({
                filterByColor: this.state.filterByColor.filter(
    ...
    nextPage = () => {
        if (
            ...
            Math.ceil(this.state.data.length / this.state.shoesPerPage)
        ) {
            this.setState({
                currentPage: this.state.currentPage + 1
            });
